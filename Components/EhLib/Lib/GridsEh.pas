...
{ TGridMoveLineEh }
  ...
  public
    ...
    procedure SetCaptureControl(ACaptureControl: TObject);
    procedure ReleaseCaptureControl;
    procedure Hide;
...
procedure TGridMoveLineEh.StartShow(Control: TControl; Pos: TPoint;
  AIsVert: Boolean; Size: Integer; ACaptureControl: TObject);
begin
  SetCaptureControl(ACaptureControl);
  FIsVert := AIsVert;
...
begin
  ...
  begin
    ...
    then
      Result := True
    else if (SelStyle = gsdsListViewThemedEh) and (goRowSelect in Options) and (gdSelected in State)
    then
      Result := True
    else
      Result := False;
...
begin
  ...
  if not IsRectEmpty(ScrollArea) and HandleAllocated then
  begin
...
begin
  ...
  if IsSmoothHorzScroll and (FTopLeft.Y = OldTopLeft.Y) then
    ...
    Exit
  else if ((FTopLeft.X <> OldTopLeft.X) or (FTopLeft.Y <> OldTopLeft.Y)) and
           not PaintLocked
  then
...
procedure TCustomGridEh.CellMouseClick(const Cell: TGridCoord; Button: TMouseButton;
...
procedure TCustomGridEh.DefaultCellMouseClick(const Cell: TGridCoord; Button: TMouseButton;
  Shift: TShiftState; const ACellRect: TRect; const GridMousePos, CellMousePos: TPoint);
begin
end;

procedure TCustomGridEh.CellMouseDown(const Cell: TGridCoord; Button: TMouseButton;
  Shift: TShiftState; const ACellRect: TRect; const GridMousePos, CellMousePos: TPoint);
var
  Processed: Boolean;
begin
...
procedure TGridMoveLineEh.ReleaseCaptureControl;
begin
  inherited Hide;
  FCaptureControl := nil;
end;

procedure TGridMoveLineEh.SetCaptureControl(ACaptureControl: TObject);
begin
  if FCaptureControl <> nil then
    raise Exception.Create('GridMoveLine already captured');
  FCaptureControl := ACaptureControl;
end;

procedure TGridMoveLineEh.Hide;
begin
  ReleaseCaptureControl;
end;
