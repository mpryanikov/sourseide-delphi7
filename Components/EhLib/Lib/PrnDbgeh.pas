...
uses PrnDGDlg, RichEdit, ComStrs, EhLibConsts, ToolCtrlsEh, DBAxisGridsEh
...
var
  ...
  PolyPolyPoints: array of TPoint;
...
//        VPrinter_Canvas.FillRect(ARect);
    ...
    begin // footer cell
      ...
      begin
        bfi := APBmFrom.bmi - DBGridEh.DataGrouping.Footers.VisibleItemsCount;
        if bfi < 0 then Exit;
        Footer := CurColumn.UsedFooter(bfi);
        AAlignment := Footer.Alignment;
        if Footer.ValueType in [fvtSum, fvtCount]
          then Value := GetFooterValue(bfi, ACol)
          else Value := DBGridEh.GetFooterValue(bfi, CurColumn);
  ...
  begin
    ...
    if ADrawGroupDataNode.NodeType <> dntDataGroupFooterEh then
      raise Exception.Create('Algorithmic error: NodeType is not dntDataGroupFooterEh type');
    CurColumn := PrnColumns[ACol];
    FooterColumnItem := ADrawGroupDataNode.DataGroupFooter.ColumnItems[CurColumn.Index];
    VarValue := ADrawGroupDataNode.FooterValues[CurColumn.Index];
    Value := FooterColumnItem.ConvertToDisplayText(ADrawGroupDataNode.FooterValues[CurColumn.Index]);
