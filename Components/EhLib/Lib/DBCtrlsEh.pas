...
{ TControlEmptyDataInfoEh }
  ...
  public
    FEditControl: TCustomDBEditEh;
    FEditButton: TEditButtonEh;
  end;

  TEditControlShowDropDownFormEventEh = procedure(EditControl: TCustomDBEditEh;
    Button: TEditButtonEh; var DropDownForm: TCustomForm;
    DynParams: TDynVarsEh) of object;

  TEditControlCloseDropDownFormEventEh = procedure(EditControl: TCustomDBEditEh;
...
{$ifdef eval}
  EhLibEvaluationForm,
{$endif}
  Dialogs, CalculatorEh, DropDownFormEh;
...
begin
  ...
  if (eb <> nil) and eb.Enabled then
    ...
    begin
      ...
      FEditButtonControlList[0].EditButtonControl.Click;
      Key := 0;
    end else
    begin
...
begin
  ...
  begin
    ...
    if inherited UseRightToLeftAlignment
      then Result.X := ClientWidth - EditRect.Right
      else Result.X := EditRect.Left;
...
//    if Msg.wParam = Integer(Sender) then
  ...
  if (Sender = FEditButtonControlList[0].EditButtonControl) then
  begin
    if not FEditButtonControlList[0].EditButtonControl.Enabled then Exit;
    if Assigned(FOnButtonDown) then
      FOnButtonDown(Sender, TopButton, AutoRepeat, Handled);
    if not Handled then
      CheckEditButtonDownForDropDownForm(EditButton, FEditButtonControlList[0].EditButtonControl, Handled);
    if not Handled then
...
//        PostMessage(Handle, CM_IGNOREEDITDOWN, Integer(TEditButtonControlEh(Sender).Tag), 0);
    ...
    for i := 1 to Length(FEditButtonControlList) - 1 do
      ...
      begin
        ...
        if not Handled then
          CheckEditButtonDownForDropDownForm(EditButtons[i - 1], FEditButtonControlList[i].EditButtonControl, Handled);
        if not Handled then
...
procedure TCustomDBEditEh.CheckEditButtonDownForDropDownForm(
