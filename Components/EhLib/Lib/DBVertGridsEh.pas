...
{ TDBVertGridSelectionEh }
  ...
  private
    ...
    FAnchorRowIndex: Integer;
    FShipRowIndex: Integer;
//    procedure SetSelectionType(ASelType: TDBVertGridSelectionTypeEh);
...
//  ,dgvhMultiSelect
  ...
  private
    ...
    FOnDataHintShow: TDBVertGridEhDataHintShowEvent;
//    FOnDrawDataCell: TDrawDataCellEvent; { obsolete }
//    FOnDrawRowCell: TDrawRowCellEvent; { obsolete }
...
uses Math, DBConsts, VDBConsts, Dialogs, Clipbrd, DBGridEhImpExp;
...
begin
  ...
  Distance := 100;
  if (FMoveMousePos.Y < 0) or (FMoveMousePos.Y >= DrawInfo.Vert.GridBoundary) then
  begin
    if FMoveMousePos.Y < 0 then
      Distance := - FMoveMousePos.Y
    else if FMoveMousePos.Y >= DrawInfo.Vert.GridBoundary then
      Distance := FMoveMousePos.Y - DrawInfo.Vert.GridBoundary;
  ...
  begin
    ...
    begin
      ...
      if Y >= DrawInfo.Vert.GridBoundary then Cell.Y := RowCount-1;
  ...
  begin
    ...
    begin
      SafeSmoothScrollData(0, -Distance);
      SelRow := TopRow;
    end else if FVertGridState = vdgsNormalEh then
      MoveRow(Row - 1, 0);
  end else if Y >= DrawInfo.Vert.GridBoundary then
  begin
    if FVertGridState in [dgsRowSelecting, dgsRectSelecting] then
    begin
      SafeSmoothScrollData(0, Distance);
      SelRow := TopRow + VisibleRowCount;
