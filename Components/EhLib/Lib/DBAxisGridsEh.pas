...
function CheckHintTextRect(DrawTextBiDiModeFlagsReadingOnly: Longint;
  ...
  WordWrap: Boolean; var TextWidth, TextHeight: Integer;
...
uses DBConsts, Dialogs, DBGridEhImpExp, Clipbrd, DropDownFormEh, Math,
...
(*    if Assigned(AxisBar.Field) then
    begin
      if AxisBar.Grid.DataLink.Active and AxisBar.LookupParams.LookupActive { (FieldKind = fkLookup)} then
      begin
      //MasterField := Dataset.FieldByName(KeyFields);
        MasterFields := TListOfFieldsEh.Create;
        try
          AxisBar.Grid.DataLink.Dataset.GetFieldList(MasterFields, AxisBar.LookupParams.KeyFieldNames);
        { AxisBar.DefaultReadonly will always be True for a lookup field.
          Test if AxisBar.ReadOnly has been assigned a value of True }
...
(*
    ...
    begin
      MasterFields := TListOfFieldsEh.Create;
      try
        Grid.DataLink.Dataset.GetFieldList(MasterFields, AxisBar.LookupParams.KeyFieldNames);
      { AxisBar.DefaultReadonly will always be True for a lookup field.
        Test if AxisBar.ReadOnly has been assigned a value of True }
        if (MasterFields.Count > 0) and MasterFieldsCanModify and
          not ((cvReadOnly in AxisBar.AssignedValues) and AxisBar.ReadOnly) then
  ...
  begin
    ...
    begin
      ...
      case EditButtonStyle of
        ...
        esPickList, esLookupDataList, esDateCalendar, esDropDown, esDataList:
        ...
        esAltCalendar, esAltPickList, esAltLookupDataList, esAltDataList: EditButtonControl.Style := ebsAltDropDownEh;
      end;
...
begin
(*  if Assigned(Field)
    then Result := LookupParams.LookupDisplayFieldName {FField.LookupResultField}
    else Result := '';*)
...
begin
  Result := not (GetBarType in [ctKeyImageList..ctCheckboxes]);
end;
...
begin
  ...
  begin
    ...
    if Result and (not AxisBar.ReadOnly) and AxisBar.CanModify(False) then
    begin
      FDatalink.Edit;
