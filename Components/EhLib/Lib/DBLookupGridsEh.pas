...
uses
  Windows, Messages, SysUtils, Classes, Controls, GridsEh,
  DBAxisGridsEh,
  DBGridEh,
  ToolCtrlsEh,
...
//{$IFDEF EH_LIB_16}
//class constructor TDBLookupGridEh.Create;
//begin
//  TCustomStyleEngine.RegisterStyleHook(TDBLookupGridEh, TScrollBoxStyleHook);
//end;
//
//class destructor TDBLookupGridEh.Destroy;
//begin
//  TCustomStyleEngine.UnRegisterStyleHook(TDBLookupGridEh, TScrollBoxStyleHook);
//end;
//{$ENDIF}
...
//  if UseRightToLeftAlignment then ChangeBiDiModeAlignment(AAlignment);
  Canvas.Brush.Color := StyleServices.GetSystemColor(DrawColumn.SpecCell.Color); //SpecRow.Color;
  AFillRect := True;
...
//  if NewHeight < GetSystemMetrics(SM_CYVSCROLL)*2 then
...
//    Style := WS_CLIPCHILDREN;
    if not Ctl3D or CustomStyleActive  then
...
procedure TPopupDataGridEh.CreateWnd;
begin
  inherited CreateWnd;
...
function TPopupDataGridEh.Focused: Boolean;
begin
  Result := True;
end;
...
{$IFDEF EH_LIB_16}
  ...
  TCustomStyleEngine.RegisterStyleHook(TPopupDataGridEh, TPopupDataGridEhStyleHook);
{$ENDIF}
finalization
